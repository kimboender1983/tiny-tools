...
export class AffiliatesService {
  ...
  async trackClick(affiliateId: string, page?: string, ip?: string, userAgent?: string): Promise<void> {
    const hashedIp = ip
  ...
  async getAnalytics(affiliateId: string, days = 30) {
    ...
    const [dailyClicks, topPages, totalClicks] = await Promise.all([
      ...
      this.clickModel.aggregate([
        { $match: { affiliate: affiliate._id, createdAt: { $gte: since }, page: { $ne: null } } },
        { $group: { _id: '$page', count: { $sum: 1 } } },
        { $sort: { count: -1 } },
        { $limit: 10 },
        { $project: { page: '$_id', count: 1, _id: 0 } },
      ]),
      this.clickModel.countDocuments({ affiliate: affiliate._id, createdAt: { $gte: since } }).exec(),
    ]);
  ...
  async getOverviewAnalytics(days = 30) {
    ...
    const totalClicks = perAffiliate.reduce((sum: number, a: { totalClicks: number }) => sum + a.totalClicks, 0);
