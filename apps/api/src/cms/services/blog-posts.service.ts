...
export class BlogPostsService {
    ...
    async create(dto: CreateBlogPostDto): Promise<BlogPostDocument> {
        ...
        const uniqueSlug = await this.slugService.ensureUnique(slug, this.blogPostModel);
    ...
    async update(idOrSlug: string, dto: UpdateBlogPostDto): Promise<BlogPostDocument> {
        const id = await this.resolveId(idOrSlug);
        const existing = await this.findById(id);

        if (dto.slug && dto.slug !== existing.slug) {
            const newSlug = this.slugService.generateSlug(dto.slug);
            dto.slug = await this.slugService.ensureUnique(newSlug, this.blogPostModel, id);
        }

        if (dto.content) {
            (dto as Record<string, unknown>).readingTime = this.calculateReadingTime(dto.content);
        }
    ...
    async toggleFeatured(idOrSlug: string): Promise<BlogPostDocument> {
        ...
        const updated = await this.blogPostModel
            .findByIdAndUpdate(id, { $set: { featured: !post.featured } }, { new: true })
            .exec();
        if (!updated) {
            throw new NotFoundException(`Blog post with id "${id}" not found`);
        }
