...
import {
  X, Save, CopyPlus, Loader2, RotateCw, RotateCcw,
  FlipHorizontal, FlipVertical, Crop, Sun, Contrast,
  Droplets, Undo2, Download,
...
async function saveAsOriginal() {
  ...
  try {
    ...
    const updated = await cms.media.replace(props.mediaItem._id, blob, props.mediaItem.filename);
    showSaveOptions.value = false;
    emit('saved', updated);
  } catch { /* */ } finally {
    saving.value = false;
