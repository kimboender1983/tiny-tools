...
<template>
  <div class="flex flex-col gap-4">
    ...
    <div class="flex flex-wrap items-center gap-2">
      ...
      <label class="inline-flex items-center gap-2 cursor-pointer select-none">
        <button
          ...
          :class="urlSafe ? 'bg-brand-500' : 'bg-[var(--color-switch-off)]'"
        ...
        <span class="text-sm text-content-tertiary">URL-safe</span>
      </label>
      ...
      <label class="inline-flex items-center gap-2 cursor-pointer select-none">
        <button
          ...
          class="relative flex h-5 w-9 items-center rounded-full px-0.5 transition-colors duration-200"
          :class="dataUri ? 'bg-brand-500' : 'bg-[var(--color-switch-off)]'"
        ...
        <span class="text-sm text-content-tertiary">Data URI</span>
      </label>
      ...
      <button
        type="button"
        class="inline-flex items-center gap-1.5 px-2.5 py-1.5 text-sm font-medium rounded-lg text-content-tertiary hover:bg-surface-secondary hover:text-content transition-colors disabled:opacity-40 disabled:cursor-not-allowed"
    ...
    >
      <!-- Input -->
      <div class="flex flex-col rounded-xl border border-surface-border overflow-hidden bg-surface" :class="isDragging ? 'ring-2 ring-brand-500/40' : ''">
        <div class="flex items-center justify-between px-4 py-2.5 border-b border-surface-border bg-surface-secondary">
          ...
          <div class="flex items-center gap-2">
            <span v-if="sourceFileName" class="text-xs text-content-muted truncate max-w-[120px]">{{ sourceFileName }}</span>
            <span v-else class="text-xs text-content-muted">{{ formatBytes(inputBytes) }}</span>
        ...
        >
          ...
          <button
            type="button"
            class="text-xs text-content-muted hover:text-content-secondary transition-colors underline"
            @click="clear"
          >
            Remove image
          </button>
        ...
        <textarea
          v-else
          v-model="input"
          class="w-full min-h-[240px] lg:min-h-[360px] p-4 font-mono text-sm leading-relaxed bg-transparent resize-y focus:outline-none text-content placeholder-placeholder"
      ...
      <div class="flex flex-col rounded-xl border border-surface-border overflow-hidden bg-surface">
        <div class="flex items-center justify-between px-4 py-2.5 border-b border-surface-border bg-surface-secondary">
          ...
          <div class="flex items-center gap-2">
            <span v-if="output" class="text-xs text-content-muted">{{ formatBytes(outputBytes) }}</span>
        ...
        <pre
          ...
          :class="output ? 'text-content' : 'text-content-faint'"
    ...
    >
      <div
        v-if="imagePreview"
        class="rounded-xl border border-surface-border overflow-hidden bg-surface"
      >
        <div class="flex items-center gap-2 px-4 py-2.5 border-b border-surface-border bg-surface-secondary">
          ...
          <span class="text-sm font-semibold text-content">Image Preview</span>
        </div>
    ...
    <div v-if="!input.trim()" class="flex flex-col gap-3">
      ...
      <div class="grid grid-cols-2 sm:grid-cols-4 gap-3">
        ...
        >
          ...
          <span class="text-xs text-content-muted truncate w-full">
