  ...
  function process() {
    ...
    try {
      ...
      if (mode.value === 'decode') {
        error.value = { message: 'Invalid Base64 string. Check for incorrect characters or padding.' };
      } else {
        error.value = { message: `Encoding failed: ${e instanceof Error ? e.message : 'Unknown error'}` };
      }
    }
