...
export function useCms() {
    ...
    const blogPosts = {
        ...
        batchUpdateStatus: (ids: string[], status: string) =>
            api.patch<{ modifiedCount: number }>(
                "/cms/blog-posts/batch/status",
                { ids, status },
                AUTH,
            ),
        allTags: () => api.get<string[]>("/cms/blog-posts/meta/tags", AUTH),
    };
    ...
    const blogWriter = {
        ...
        tones: {
            ...
            create: (data: Partial<IWritingTone>) =>
                api.post<IWritingTone>("/cms/blog-writer/tones", data, AUTH),
            update: (id: string, data: Partial<IWritingTone>) =>
                api.put<IWritingTone>(`/cms/blog-writer/tones/${id}`, data, AUTH),
            delete: (id: string) => api.delete<void>(`/cms/blog-writer/tones/${id}`, AUTH),
        },
        topics: {
            list: () => api.get<ITopicQueue[]>("/cms/blog-writer/topics", AUTH),
            create: (data: Partial<ITopicQueue>) =>
                api.post<ITopicQueue>("/cms/blog-writer/topics", data, AUTH),
            update: (id: string, data: Partial<ITopicQueue>) =>
                api.put<ITopicQueue>(`/cms/blog-writer/topics/${id}`, data, AUTH),
            reset: (id: string) =>
                api.patch<ITopicQueue>(`/cms/blog-writer/topics/${id}/reset`, {}, AUTH),
        ...
        scheduler: {
            ...
            update: (data: Partial<ISchedulerConfig>) =>
                api.put<ISchedulerConfig>("/cms/blog-writer/scheduler", data, AUTH),
        },
    };
