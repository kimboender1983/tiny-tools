...
function getOutputExtension(format: OutputFormat, _originalName: string): string {
...
export function useImageCompressor() {
    ...
    const hasCompressedImages = computed(() => images.value.some((img) => img.status === "done"));

    const hasPendingImages = computed(() => images.value.some((img) => img.status === "pending"));
                ...
                try {
                    ...
                    canvas.toBlob(
                        (blob) => {
                            ...
                            if (img.compressedPreviewUrl)
                                URL.revokeObjectURL(img.compressedPreviewUrl);
            ...
            try {
                ...
                worker.onmessage = (event) => {
                    ...
                    if (data.type === "progress") {
                        ...
                        const blob = new Blob([data.compressedData], {
                            type: data.compressedMimeType,
                        });
                        img.compressedBlob = blob;
                        img.compressedSize = blob.size;
    ...
    async function downloadAllAsZip() {
        const doneImages = images.value.filter(
