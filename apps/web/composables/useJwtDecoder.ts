...
export function useJwtDecoder() {
  ...
  function decode() {
    ...
    if (parts.length !== 3) {
      decoded.value = null;
      error.value = { message: `Invalid JWT structure: expected 3 parts separated by dots, found ${parts.length}` };
      securityWarnings.value = [];
      timestamps.value = {};
      expiryStatus.value = null;
      saveState();
    ...
    try {
      ...
      try {
        ...
        decoded.value = null;
        error.value = { message: 'Failed to decode payload: invalid base64url or malformed JSON' };
        securityWarnings.value = [];
        timestamps.value = {};
        expiryStatus.value = null;
        saveState();
      ...
      if (typeof payload.exp === 'number') {
        ...
        expiryStatus.value = {
          expired,
          label: expired ? `Expired ${tsInfo.relative}` : `Valid for ${tsInfo.relative.replace('in ', '')}`,
        };
      } else {
        expiryStatus.value = null;
      }
      ...
      decoded.value = null;
      error.value = { message: 'Failed to decode header: invalid base64url or malformed JSON' };
      securityWarnings.value = [];
      timestamps.value = {};
      expiryStatus.value = null;
    }
