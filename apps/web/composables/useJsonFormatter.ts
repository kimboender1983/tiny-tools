...
function humanizeJsonError(raw: string): string {
    ...
    const msg = raw.replace(/^(SyntaxError:\s*)?/i, "");

    // Common rewrites
    if (/unexpected token\s*}/i.test(msg))
        return "Unexpected closing brace — possible trailing comma or missing value";
    if (/unexpected token\s*]/i.test(msg))
        return "Unexpected closing bracket — possible trailing comma or missing value";
    if (/unexpected token\s*'/i.test(msg))
        return "Single quotes are not valid JSON — use double quotes instead";
    if (/unexpected end of/i.test(msg))
        return "Unexpected end of input — the JSON is incomplete or has unmatched brackets";
    if (/unexpected token.*in json at position 0/i.test(msg))
        return "Input does not start with valid JSON — expected { or [";
    ...
    function process() {
        ...
        try {
            ...
            if (minifyMode.value) {
                ...
                formattedOutput.value = JSON.stringify(
                    parsed,
                    null,
                    getIndentString(indentStyle.value),
                );
            }
            ...
            if (raw.length < 50_000) {
                ...
                if (treeData.value.children) {
                    treeData.value.children.forEach((c) => {
                        c.expanded = false;
                    });
                }
            }
