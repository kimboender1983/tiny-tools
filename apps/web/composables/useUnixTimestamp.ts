...
export function useUnixTimestamp() {
  ...
  function process() {
    ...
    if (direction.value === 'toDate') {
      ...
      if (isNaN(date.getTime())) {
        error.value = { message: 'Invalid date. Try formats like "2026-04-03", "Apr 3 2026 14:30", or ISO 8601.' };
        parsed.value = null;
        saveState();
        return;
      }
  ...
  function formatInTimezone(date: Date, tz: string): TimezoneEntry {
    ...
    try {
      ...
      return { id: tz, label: tzDef.label, city: tzDef.city, abbr: '', offset: '', time: '—', date: '—' };
    }
  }
